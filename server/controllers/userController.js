...
class UserController {
    static register = async (req, res) => {
        try {
            ...
            if (password !== repeatPassword) {
                return res.status(203).json({ message: "Password & Confirm Password not match 😊" });
            }
            const user = await userModel.findOne({ email });
            if (user) {
                return res.status(203).json({ message: "This Email already exit 😊" });
            }
    ...
    static login = async (req, res) => {
        try {
            ...
            const user = await userModel.findOne({ email });
            if (!user) {
                return res.status(203).json({ message: "This Email does not exit 😊" });
            }
